...
class TranscribeTranscript():
    ...
    def __init__( self, transcript_sj, isoform_id, strand_sign, path_genomeidx, hash_transcript_notes ):
        """
        Args:
            ...
            -isoform_id = string that is the isoform ID of interest. This is essential for SpliceJunction's def spliced_elems_position_range()
        ...
        """
        self.transcript_sj = sorted( transcript_sj, key = lambda s: s.start, reverse = False )
        self.isoform_id = isoform_id        #essential for SpliceJunction's def spliced_elems_position_range()
        self.list_exons = TranscribeTranscript.retrieve_exon_from_sj_v2( self.transcript_sj, self.isoform_id )
    ...
    def retrieve_exon_from_sj_v2( list_sj, isoform_id ):
        ...
        Function: same as the function retrieve_exon_from_sj(), but returns a list of Exon instances
        """
        #retrieve all exons spliced together by all SpliceJunction instances
        list_exons = []
        spliced_exons = list_sj[0].spliced_elems_position_range_v2( isoform_id, False, True )
        if spliced_exons['exon_start']:
            list_exons.append( spliced_exons['exon_start'] )
        for each_sj in list_sj:
            #find the exons spliced together
            spliced_exons = each_sj.spliced_elems_position_range_v2( isoform_id, False, True )
            if spliced_exons['exon_end']:
                list_exons.append( spliced_exons['exon_end'] )
...
class TranslateTranscript( TranscribeTranscript ):
    ...
    def __init__( self, transcript_sj, isoform_id, strand_sign, path_genomeidx, hash_transcript_notes ):
        TranscribeTranscript.__init__( self, transcript_sj, isoform_id, strand_sign, path_genomeidx, hash_transcript_notes )
