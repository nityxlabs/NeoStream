...
##------------- CONSTANT COLUMNS FOR FILE - NOT FOR THIS VERSION IF IsoformSJ
...
##COLUMNS FOR UNIQUE READS (path in Atlas: /home/mokha/Documents/Krauthammer_Lab/160427_SJFalsePositives/Data/Tabix_Unique_Count)
# header = "chrom\tpos_start\tpos_end\tstrand\t"
# header+= "gene_sym\tgene_strand\tassigned_isoforms"
# header+= "tophat_count\tall_counts\tunique_count\t"     #tophat_count = count recorded in junctions.bed file, all_counts = unique & non-uniquely-mapped reads retrieved by pysam, unique_counts = uniquely-mapped reads mapped by pysam
# header+= "sj_id\tsj_strand_sign\tsj_canonical\tstat_exon_skip\texon_skip\tframe_preserved\treading_frame\t"
# header+= "samples_contain_sj\tprevalence_all\tprevalence_control\n"

COLSJ_CHROM = 0
COLSJ_LEFTMOST = 1
COLSJ_RIGHTMOST = 2
COLSJ_SS = 3      #COLSJ_SS  =  column splice junction strand sign
COLSJ_GENE_SYM = 4
COLSJ_GENE_SS = 5       #COLSJ_GENE_SS = column gene strand sign
COLSJ_ASSIGN_ISOFORM = 6
COLSJ_TOPHAT_COUNT = 7      #COLSJ_TOPHAT_COUNT = read count reported by tophat
COLSJ_ALL_COUNT = 8      #COLSJ_ALL_COUNT =  all reads (unique & multimapped) that map to region
COLSJ_UNIQ_COUNT = 9      #COLSJ_UNIQ_COUNT =  unique reads that map to region
COLSJ_SJID = 10
COLSJ_SJ_SS = 11      #COLSJ_SJ_SS  =  column splice junction strand sign, isn't this the same as COLSJ_SS
COLSJ_CANON = 12
COLSJ_ES_STAT = 13       #COLSJ_ES = status if exon skipping occurs (TRUE = exons skipped, FALSE = no exon skip)
COLSJ_ES = 14       #COLSJ_ES = exons skipped occurs
COLSJ_RF_PRESERVED = 15       #COLSJ_RF_PRESERVED = status if reading frame is preserved (TRUE = reading frame preserved, FALSE = frameshift occurred)
COLSJ_RS = 16       #COLSJ_FS = reading frame (in format num1,2 - e.g. 1,2 (this is in-frame) OR 0,2 (this is out-of-frame))
COLSJ_SAMPLES_CONTAIN = 17      #COLSJ_SAMPLES_CONTAIN = Samples that contain this splice junction (list of samples, e.g. samp_a,samp_b,...)
COLSJ_PREVALENCE_ALL = 18      #COLSJ_PREVALENCE_ALL = the total number of samples that contain this splice junction
COLSJ_PREVALENCE_CTRL = 19     #COLSJ_PREVALENCE_CTRL = the number of control samples that contain this splice junction

ROWLEN_MAX = COLSJ_PREVALENCE_CTRL

exon_base = 1           #if 0, then exons are 0-based, if 1, then exons are 1-based

class IsoformSJ_UniqFile( Isoform ):
    ...
    def __init__( self, isoform_id, list_sj, sj_thres = -2, hash_pos = None, simulant_sj = False, group_sj = 0 ):
        """
        Args:
            -isoform_id = string that is the isoform ID to recreate the transcript
            -list_sj = array of SpliceJunction instances that will be used to reconstruct transcripts
        ...
        super( IsoformSJ_UniqFile, self ).__init__( isoform_id, hash_pos )
    ...
    @staticmethod
    def get_sj_tabix_file( path_sample_tabix, pos_range, gene_sym, sj_thres = 0 ):
        """
        Args:
        ...
        try:
            ...
            print "Error in IsoformSJ_UniqFile.sj_in_range: Tabix Error with range ", pos_range
            return []


        #retrieve all aberrant splice junctions
        hash_aberrant_prevalence = IsoformSJ_UniqFile.aberrant_prevalence( pos_range )       #key = string (format: 'chrom:start-end') & value = tuple where [0] = sample prevalence & [1] = control prevalence
        ...
        for i, each_sj in enumerate( get_sj ):
            ...
            obj_sj = SpliceJunction( each_sj[COLSJ_SJID], each_sj[COLSJ_CHROM], true_pos[0], true_pos[1], each_sj[COLSJ_LEFTMOST], each_sj[COLSJ_RIGHTMOST], each_sj[COLSJ_SS], int( each_sj[COLSJ_UNIQ_COUNT] ), gene_sym, sample_prevalence, control_prevalence )
            list_sj.append( obj_sj )

            ##TEST:: print "IsoformSJ_UniqFile.sj_in_range: ", i, " of ", tabix_counter, " & start = ", obj_sj.start, " & end = ", obj_sj.end, " & canon = ", obj_sj.canon, " & isoform = ", obj_sj.assigned_isoform
    ...
    def get_prevalence( arr_tabix_sj ):
        ...
        prevalence = IsoformSJ_UniqFile.aberrant_prevalence( sj_range )     #retrieve hash where k = genomic range & value = returns tuple where [0] = sample prevalence & [1] = control prevalence
    ...
    @staticmethod
    def create_sj_list( list_tuple_pos, chrom, strand, gene_sym ):
        """
        Args:
        ...
        for i, sj in enumerate( list_tuple_pos ):
            id = 'SIMSJ_' + str( i )
            record_sj.append( SpliceJunction( id, chrom, sj[0], sj[1], None, None, strand, -1, gene_sym ) )

        return record_sj

    def create_canon_transcript( self ):
        """ Function: create canonical version of the transcript """
        #retrieve all donor-acceptor sites (canonical end points for SJ) -> append 
        da_sites = self.get_donor_acceptor_sites( True )
        transcript_sj = []
        for i, sj in enumerate( da_sites ):
            id = 'SIMSJ_' + str( i )
            transcript_sj.append( SpliceJunction( id, self.chrom, sj[0], sj[1], None, None, self.strand, -1, self.gene_sym ) )
    ...
    def sj_canon_absent( self, create_sj = True ):
        ...
        for i, da_site in enumerate( da_sites ):
            ...
            if not canon_sj:
                ...
                id = 'SIMSJ_' + str( i )
                elem = SpliceJunction( id, self.chrom, da_site[0], da_site[1], None, None, self.strand, -1, self.gene_sym ) if create_sj else id
    ...
    def is_isoform_present( self, threshold_missing = 0, start_end = 0 ):
        ...
        # da_sites = self.get_donor_acceptor_sites()
        # print "IsoformSJ_UniqFile.isoform_present: canon_absent = ", len( num_canon_absent ), " & threshold = ", threshold, " & # of SJ = ", len( self.hashExonList ), " & last_exon_num = ", self.last_exon_num, " & len( da_sites ) = ", len( da_sites )
    ...
    ##MAY DELETE THIS - build_transcript_around_sj()
    def reconstruct_transcript_single_sj( self ):
        """
        ...
        list_simulants = IsoformSJ_UniqFile.create_sj_list( da_sites, hash_sj_pos['chrom'], convert_ss, arr_rc[COL_GENE_SYM] )

        #Go through each aberrant SJ
        all_noncanon_sj = [x for x in self.list_sj if not x.canon]
        for each_sj in all_noncanon_sj:
            list_sj_oi = list_simulants[:]      #make sure to make a duplicate as to not affect original 'list_simulants'
            list_sj_oi.append( each_sj )
            t_start = self.build_start_v2( list_sj_oi, False )

            t_full = self.build_transcript_v2( t_start, list_sj_oi, [], max_penalty, 0, canon_only )       #t_full = array of SJ instances that constitute an mRNA transcript
    ...
    def build_transcript_around_sj( self, obj_sj ):
        ...
        Function: will build a transcript around SpliceJunction Object 'obj_sj'
        Test Cases:
        ...
        simulant_sj = IsoformSJ_UniqFile.create_sj_list( da_sites, self.chrom, self.strand, self.gene_sym )
        ...
        #build starting beginning of transcript
        exon_oi = sj_exons[0]
        bool_continue = True
        while( bool_continue ):
    ...
    def build_start_v2( self, list_sj, canon_only = False ):
        ...
        spliced_exons = []      #array of tuples that records all spliced exons, where each element is a tuple, [0] = start exon, [1] = end exon
        for each_sj in list_sj:
            spliced_exons.append( each_sj.spliced_elems( self.isoform_id, False ) )

        #find the "first exon" - exon where there is no splice junction
        all_start_exons = [x[0] for x in splice_exons] if self.strand > 0 else [x[1] for x in splice_exons]
        all_end_exons = [x[1] for x in splice_exons] if self.strand > 0 else [x[0] for x in splice_exons]
        uniq_start_exons = list( set(all_start_exons) - set(all_end_exons) )

        #record all SJs that contain these unique start exons
        first_sj = []       #records all SJs that contain these unique start exons
        for start_exon in uniq_start_exons:
            #create a new array for each new start splice junction
            direction = 1 if self.strand == 1 else 2        #if + gene, then starting exon will have lowest numerical genomic position, else if - gene, then starting exon will have highest numerical genomic position
            first_sj.append( self.exon_to_sj_v2(start_exon, list_sj, direction, canon_only) )
        ...
        # #create a new array for each new start splice junction
        # direction = 1 if self.strand == 1 else 2        #if + gene, then starting exon will have lowest numerical genomic position, else if - gene, then starting exon will have highest numerical genomic position
        # first_sj = self.exon_to_sj( first_exon, direction, canon_only )
