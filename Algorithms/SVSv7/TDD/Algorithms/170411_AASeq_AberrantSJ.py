...
def get_nmd_irrelevant_region( obj_tt, pos_aberrant ):
    ...
    if obj_tt.iso_sj.strand < 0:
        ...
        ##TEST::
        print "NMD Irrelvant Seq MINUS STRAND: "
        print "NMD Irrelevant Seq MINUS STRAND: chrom = ", obj_tt.iso_sj.chrom, "& start = ", obj_tt.arr_genome_pos[0], " & end = ", obj_tt.arr_genome_pos[i_tp_start + 1], " & pos_translate_start = ", pos_translate_start, " & pos_tp_start = ", pos_tp_start

    else:
        nmd_irrelevant_seq = ''.join( obj_tt.arr_nuc_seq[i_tp_start:] )     #retrieve from tp_start until the last nucleotide

        ##TEST::
        print "NMD Irrelevant Seq PLUS STRAND: chrom = ", obj_tt.iso_sj.chrom, "& start = ", obj_tt.arr_genome_pos[i_tp_start], " & end = ", obj_tt.arr_genome_pos[-1], " & pos_translate_start = ", pos_translate_start, " & pos_tp_start = ", pos_tp_start
...
print "------------ TDD: 170411_AASeq_AberrantSJ.py ------------"
"""
Algorithm: this algorithm is similar to TDD 170403_AASeq_NormalMutant.py & 170411_AASeq_Mutation.py, where it retrieves the amino acid sequence before an aberrant splicing event. Whereas 170403_AASeq_NormalMutant.py tests TranscribeTranscript functions find_codon_beginning_prev() & find_aa_pos_before(), this algorithm tests def find_codon_beginning_nearest() & find_aa_pos_surrounding()
...
#TEST SJ 6 - 
sj_id = 'SJ_1'
sj_range = 'chr19:36507862-36510135'
strand = -1
read_count = 10
gene_sym = 'CLIP3'
isoform_id = 'NM_001199570' 
sample_prevalence = 1
control_prevalence = 0
...
if bool_possible:
    ...
    num_aa = 8
    bool_before = True
    hash_range_aa_before_mut = obj_tt.find_aa_pos_surrounding( pos_aberrant, num_aa, bool_before )

    ##TEST::
    print "Result of obj_tt.find_aa_pos_surrounding() = ", hash_range_aa_before_mut
    ...
    print "CANON BEFORE SJ: position of AA before - ", str_genome_pos_aa_before_mut
    print "CANON BEFORE SJ: nuc_seq_aa_before_mut = ", nuc_seq_aa_before_mut
    print "CANON BEFORE SJ: protein_aa_before_mut = ", protein_aa_before_mut, " & len = ", len( protein_aa_before_mut )
    print "CANON BEFORE SJ: hash for AA before SJ = ", hash_range_aa_before_mut
    print "************\n"
    print "nucleotide nmd_sensitive_seq = ", nmd_sensitive_seq
    print "protein_nmd_sen = ", protein_nmd_sen, " & len = ", len_nmd_sen
    print "~~~~~~~~~~~~\n"
    print "nucleotide nmd irrelevant = ", nmd_irrelevant_seq
    print "protein_nmd_irrel = ", protein_nmd_irrel, " & len = ", len_nmd_irrel
