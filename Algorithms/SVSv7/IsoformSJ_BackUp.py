...
class IsoformSJ( Isoform ):
    ...
    def __init__( self, isoform_id, path_sample_tabix, sj_thres = 0, simulant_sj = False, group_sj = 0 ):
        """
        Args:
        ...
        """
        super( IsoformSJ, self ).__init__( isoform_id )
        self.sj_thres = sj_thres
        self.path_sample_tabix = path_sample_tabix

        # self.canon_transcript = self.create_canon_transcript()           #array of Splice Junction objects that constitutes a canonical transcript

        #extract position range 
        isoform_range = str( self.chrom ) + ':' + str( self.boundary[0] ) + '-' + str( self.boundary[1] )
        
        self.list_sj = self.sj_in_range( path_sample_tabix, isoform_range, self.gene_sym, self.sj_thres )
    ...
    @staticmethod
    def sj_in_range( path_sample_tabix, pos_range, gene_sym, sj_thres = 0 ):
        """
        Args:
            path_sample_tabix = string that is the path to a tabix-indexed file of the sample's junctions.bed file
            pos_range = string that is format 'chrom:start-end' - will be used by tabix.querys() to find any elements within range
            sj_thres = read count threshold that will be used to filter "noisy, background" splice junctions from "signal" splice junctions
        Function: retrieves splice junctions within file that are within a genomic range 'pos_range'
        """
        ...
        for i, each_sj in enumerate( get_sj ):
            ...
            obj_sj = SpliceJunction( each_sj[COLSJ_SJID], each_sj[COLSJ_CHROM], true_pos[0], true_pos[1], each_sj[COLSJ_LEFTMOST], each_sj[COLSJ_RIGHTMOST], each_sj[COLSJ_SS], int( each_sj[COLSJ_READCOUNT] ), gene_sym, sample_prevalence, control_prevalence )
    ...
    def exon_to_sj( self, exon, direction = 0, canon = False ):
        """
        Args:
    ...
    def get_sj_within_range( self, pos_range, inclusive_type = 3, canon_mode = 0 ):
        ...
        Args:
            ...
            -inclusive_type = integer that conveys the type of inclusion to consider
                -1 = only get SJs where the start position is within the boundary
                -2 = only get SJs where the end position is within the boundary
                -3 = only get SJs where the start and end position is within the boundary
            -canon_mode = integer taht conveys the type of canonical 
                -0 = will return all SJs, canonical & non-canonical, within the range
                -1 = will only return canonical SJs within the range
                -2 = will only return canonical SJs within the range and the same isoform ID is assigned
    ...
    def transcript_exon_str( self, transcript_sj, display_stat = 1, allow_none = True ):
        ...
        Args:
            ...
            allow_none = boolean (this is for the SpliceJunction def spliced_exons_position_range():
                -if True, then will still output string even if either of the exons from self.spliced_exons() returns None
        ...
        """
        ##INTERESTING - when a function returns a hash, I can immediately call on one of the elements in the hash after the function declaration (i.e. x.spliced_exons_position_range( self.isoform_id, False, allow_none )['str_sj_pos'])
        if display_stat == 2:       #display genomic positions
            list_pos = [ x.spliced_exons_position_range( self.isoform_id, False, allow_none )['str_sj_pos'] for x in transcript_sj ]
        elif display_stat == 3:
            list_pos = [ str(x) for x in transcript_sj ]
        elif display_stat == 4:
            list_pos = [ x.sj_id for x in transcript_sj ]
        else:               #display element numbers (e.g. exon 9)
            list_exons = [ x.spliced_exons( self.isoform_id, False ) for x in transcript_sj ]
            list_pos = []
    ...
    def reconstruct_transcript( self, canon_only = False, max_penalty = 0 ):
        ...
        #         exons = each_sj.spliced_exons( self.isoform_id )
    ...
    def build_transcript( self, t_temp, t_full, max_penalty, counter, canon_only = False ):
        ...
        for t in t_temp:        #t = array of Splice Junction objects that constitute a transcript
            ...
            sj_exons = t[-1].spliced_exons( self.isoform_id )       #returns tuple of Exon objects, where [0] = lower numerical exon & [1] = higher numerical exon
