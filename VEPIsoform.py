...
class Isoform( object ):
    ...
    def __init__( self, isoform_id, db_type, hash_pos = None ):
        """
        Args:
        ...
        self.tss = info_isoform.tss()       #a tuple with 2 elements, the position of the transcription start site. Usually both positions are the same number.
    ...
    @staticmethod
    def closest_isoform_variant_pos( chrom, start, end ):
        """
        Args:
        ...
        get_closest_variant = Isoform.closest_isoform_variant_process( isoform_variants, chrom, start )
        return get_closest_variant


    @classmethod
    def is_obj_possible( cls_obj, isoform_id, pos_range ):
        """
        Args:
            -isoform_id = string that is the isoform ID
            -pos_range = string in the format 'chrom:start-end'. This will usually be the end positions of a splicing event
            -gene_db = integer that chooses the database of interest
                -1 = uses Isoform.obj_cruzdb.refGene
                -2 = uses Isoform.obj_cruzdb.ensGene
        ...
        get_isoform_variant = Isoform.closest_isoform_variant_check( isoform_variants, hash_genomic_pos['chrom'], hash_genomic_pos['start'] )
        ...
        get_isoform_variant = Isoform.closest_isoform_variant_check( isoform_variants, hash_genomic_pos['chrom'], hash_genomic_pos['end'] )
    ...
    def closest_isoform_variant_isoform( cls_obj, isoform_id, db_type, chrom, pos_oi ):
        ...
        return cls_obj.closest_isoform_variant_process( isoform_variants, chrom, pos_oi )

    @classmethod
    def closest_isoform_variant_process( cls_obj, isoform_variants, chrom, pos_oi ):
        """
        Args:
        ...
        """
        get_isoform_variant = cls_obj.closest_isoform_variant_check( isoform_variants, chrom, pos_oi )
    ...
    @classmethod
    def closest_isoform_variant_check( cls_obj, isoform_variants, chrom, pos_oi ):
        """
        Args:
            isoform_variants = array of all isoforms found using cruzdb.Genome.bin_query or cruzdb.Genome.refGene.filter_by
        ...
        for i, each_isoform in enumerate( isoform_variants ):
            ...
            hash_gene_sym[i] = each_isoform.name if self.db_type == 3 else each_isoform.name2
    ...
    ##NEED TO DELETE THIS --> replaced by "def get_element_2()"
    ...
    def is_canon_sj( self, start, end, other_annots = False ):
        """ 
        Args:
        ...
        da_refseq = self.donor_acceptor_sites       #da_refseq = Donor-Acceptor Sites for RefSeq
        if sj_pos in da_refseq:
            sj_canon =  True
        else:
            sj_canon = False
        ...
        if other_annots:
            da_ensembl = self.ensembl_get_donor_acceptor_sites()        #da_refseq = Donor-Acceptor Sites for RefSeq
    ...
    @staticmethod
    def total_mapped_reads( bam_path ):
        """
        Args:
            bam_path = string that is the path to the .bam file
        ...
        """
        mapped_reads = [ int( x.split('\t')[2] ) for x in pysam.idxstats( bam_path ) ]
